...
at::Tensor ROIAlignForwardCPU(
  ...
  auto output = torch::zeros({num_rois, channels, pooled_height, pooled_width}, input.options()); // auto output = input.type().tensor({num_rois, channels, pooled_height, pooled_width});
  ...
  int64_t sampling_ratio)
{
  ...
  // Output at::Tensor is (num_rois, C, pooled_height, pooled_width)
  auto grad_in = torch::zeros({b_size, channels, height, width}, bottom_rois.options()); // auto grad_in = bottom_rois.type().tensor({b_size, channels, height, width}).zero_(); 
