...
def lcd_eval(args):
    ...
    # Load dataset
    imgs, gt = get_dataset(args.lcd_dataset, args.debug)
    if args.plot_gt:
        plt.figure()
        plt.imshow(gt, cmap='gray', interpolation='nearest')
        plt.savefig('{}_gt_plot.png'.format(args.lcd_dataset), format='png', dpi=150)
        plt.show()
        sys.exit(0)
    ...
    if args.sweep_median:
        k_size = None
    elif args.lcd_dataset.lower() == 'city':
        k_size = 17  # BEST HARD-CODED PARAMETER FROM SWEEP: ```range(1,61,2)```
    elif args.lcd_dataset.lower() == 'college':
    ...
    sim = similarity_matrix(descs, gt, plot=True, cluster=args.cluster, median=True,
                            k_size=k_size, name='_'.join([args.lcd_dataset, model_name]))
